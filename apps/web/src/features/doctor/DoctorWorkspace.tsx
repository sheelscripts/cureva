...
export default function DoctorWorkspace({ currentSubView, resetTrigger }: DoctorWorkspaceProps) {
  ...
  function renderHomeOverview() {
    return (
      <div className="flex-1 p-4 md:p-6 lg:p-8 space-y-5 md:space-y-6 overflow-y-auto bg-bg-base">
        {/* Dynamic Greeting Banner */}
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
          <div>
            <h1 className="text-xl md:text-2xl lg:text-3xl font-display font-medium text-text-primary tracking-tight">
          ...
          <div className="flex items-center gap-2 px-3 py-1.5 rounded-sm bg-accent/5 border border-accent/20 shrink-0">
            <span className="h-2 w-2 rounded-full bg-status-safe animate-pulse shrink-0" />
            <span className="text-[11px] font-mono text-accent uppercase tracking-wider font-semibold whitespace-nowrap">
              Session active
            </span>
          </div>
        </div>
        ...
        {/* KPI Strip — 2-col on mobile, 4-col on desktop */}
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 bg-bg-surface p-4 md:p-6 rounded-sm border border-border-dim shadow-xs">
          <div>
            <span className="text-[9px] font-sans font-bold tracking-widest text-text-tertiary uppercase block leading-tight">
              Revenue Protected
            </span>
            <span className="text-2xl md:text-3xl font-mono font-bold text-accent block mt-1.5">
              ₹42,000
            </span>
            <span className="text-[10px] font-sans text-status-safe mt-1 block">↑ 12% vs last cycle</span>
          </div>
          ...
          <div>
            ...
            <span className="text-[10px] font-sans text-status-safe mt-1 block">Goal (80%+) met</span>
          </div>
          ...
          <div>
            ...
            <span className="text-[10px] font-sans text-text-secondary mt-1 block">4 done · 8 pending</span>
          </div>
          ...
          <div>
            ...
            <span className="text-2xl md:text-3xl font-mono font-bold text-status-danger block mt-1.5">
              3
            </span>
            <span className="text-[10px] font-sans text-text-tertiary mt-1 block">Needs triage</span>
          </div>
        </div>
        ...
        <div className="flex flex-col lg:flex-row gap-6">
          <div className="flex-1 space-y-6">
            ...
            <div className="bg-bg-surface p-6 rounded-sm border border-border-dim shadow-xs">
              ...
              {/* Queue list — card style on mobile, table on md+ */}
              <div className="space-y-0 divide-y divide-border-dim/40">
                {queue.slice(0, 5).map((appt) => (
                  <div key={appt.appointmentId} className="py-3 flex items-center gap-3">
                    {/* Time */}
                    <div className="font-mono text-xs text-text-secondary shrink-0 w-[52px]">
                      {appt.time}
                    </div>
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-1.5 flex-wrap">
                        <button
                          ...
                          className="text-xs font-semibold text-text-primary hover:text-accent hover:underline text-left leading-tight"
                        >
                          {appt.patientName}
                        </button>
                        <span className="text-[9px] font-mono text-text-tertiary bg-bg-subtle px-1 rounded-xs uppercase">
                          {appt.age}{appt.gender}
                        </span>
                      </div>
                      <p className="text-[10px] text-text-tertiary truncate mt-0.5 hidden sm:block">{appt.reason}</p>
                    </div>
                    ...
                    <div className="shrink-0">
                      <span className={`text-[9px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-xs whitespace-nowrap ${
                        ...
                        {appt.status === 'in-progress' ? 'Active' : appt.status}
                      </span>
                    </div>
                    ...
                    <div className="shrink-0">
                      {appt.status === 'completed' ? (
                        <button
                          ...
                          className="text-[10px] font-bold text-text-tertiary hover:text-text-primary whitespace-nowrap"
                        >
                          Notes
                        </button>
                      ) : (
                        <button
                          ...
                          className="bg-accent text-bg-base text-[10px] font-bold px-2 py-1 rounded-sm hover:opacity-90 inline-flex items-center gap-1 whitespace-nowrap"
                        >
                          <Mic size={9} /> Scribe
                        </button>
                      )}
              ...
              {/* SlotSaver Protection Status Widget */}
              <div className="mt-6">
                <SlotSaverWidget />
              </div>
            </div>
  ...
  function renderQueueList() {
    return (
      <div className="flex-1 p-4 sm:p-5 md:p-8 space-y-4 md:space-y-6 overflow-y-auto bg-bg-base">
        ...
        {/* Queue — card list on mobile, table on md+ */}
        <div className="bg-bg-surface rounded-sm border border-border-dim overflow-hidden">
          <div className="p-3 md:p-4 border-b border-border-dim bg-bg-base/30 flex justify-between items-center gap-2">
            <span className="text-xs font-sans font-semibold text-text-secondary">
              Active Triage View Controls
            </span>
            <span className="text-[10px] font-sans text-text-tertiary hidden sm:block">
              Click patient name to view full 360 history
            </span>
          </div>

          {/* MOBILE: card list */}
          <div className="divide-y divide-border-dim/40 md:hidden">
            {queue.map((appt) => (
              <div key={appt.appointmentId} className="p-3 space-y-2">
                <div className="flex items-start justify-between gap-3">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-1.5 flex-wrap">
                      <button
                        onClick={() => handleSelectPatientFull(appt.patientId)}
                        className="text-sm font-semibold text-text-primary hover:text-accent hover:underline text-left"
                      >
                        {appt.patientName}
                      </button>
                      <span className="text-[9px] font-mono text-text-tertiary bg-bg-subtle px-1.5 py-0.5 rounded-xs uppercase">
                        {appt.age}{appt.gender}
                      </span>
                    </div>
                    <p className="text-[10px] text-text-secondary mt-0.5 truncate">{appt.reason}</p>
                  </div>
                  <div className="shrink-0 text-right space-y-1">
                    <div className="font-mono text-xs text-text-secondary">{appt.time}</div>
                    <span className={`text-[9px] font-bold uppercase px-2 py-0.5 rounded-xs block text-center ${
                      appt.status === "in-progress"
                ...
                <div className="flex items-center justify-between gap-2">
                  <div className="flex items-center gap-2">
                    {renderRiskBadge(appt.riskScore)}
                    {appt.waitMinutes !== null && (
                      <span className="text-[10px] text-text-tertiary font-mono">{appt.waitMinutes}m wait</span>
                    )}
                  </div>
                  <div className="flex items-center gap-2">
                    <button
                      onClick={() => handleSelectPatientFull(appt.patientId)}
                      className="text-[10px] font-semibold text-text-tertiary hover:text-text-primary"
                    >
                      Chart
                    </button>
                    <button
                      onClick={() => handleStartScribe(appt.appointmentId, appt.patientId)}
                      className="bg-accent text-bg-base text-[10px] font-bold px-2.5 py-1 rounded-sm hover:opacity-90 inline-flex items-center gap-1"
                    >
                      <Mic size={10} /> Scribe
                    </button>
                  </div>
          ...
          <table className="w-full text-left min-w-[700px]">
            <thead>
              <tr className="border-b border-border-dim text-[10px] font-sans font-bold text-text-tertiary uppercase tracking-wider h-10">
                <th className="pl-4 whitespace-nowrap">TIME</th>
                <th className="whitespace-nowrap">APPT ID</th>
                <th className="whitespace-nowrap">PATIENT</th>
                <th className="whitespace-nowrap">REASON</th>
                <th className="whitespace-nowrap">WAIT</th>
            ...
            <tbody className="divide-y divide-border-dim/50">
              {queue.map((appt) => (
                <tr key={appt.appointmentId} className="hover:bg-bg-subtle/20 h-12 transition-colors">
                  <td className="pl-4 font-mono text-xs text-text-secondary whitespace-nowrap">
                    {appt.time}
                  </td>
                  <td className="font-mono text-[10px] text-text-tertiary whitespace-nowrap">
                    {appt.appointmentId}
                  </td>
                  <td>
                    <div className="flex items-center gap-1.5">
                      <button
                        onClick={() => handleSelectPatientFull(appt.patientId)}
                        className="text-xs font-semibold text-text-primary hover:text-accent hover:underline text-left whitespace-nowrap"
                      >
                        {appt.patientName}
                      </button>
                      <span className="text-[9px] font-sans bg-bg-subtle px-1 rounded-xs uppercase text-text-tertiary whitespace-nowrap">
                        {appt.age}{appt.gender}
                      </span>
                    </div>
                  </td>
                  <td className="text-xs font-sans text-text-secondary max-w-[220px] truncate">
                    {appt.reason}
                  </td>
                  <td className="font-mono text-xs text-text-secondary whitespace-nowrap">
                    {appt.waitMinutes !== null ? `${appt.waitMinutes}m` : "–"}
                  </td>
                  <td className="font-mono text-xs">
                    {renderRiskBadge(appt.riskScore)}
                  </td>
                  <td>
                    <span className={`text-[10px] font-bold px-2 py-0.5 rounded-xs uppercase tracking-wider whitespace-nowrap ${
                      appt.status === "in-progress"
                        ? "bg-status-info/10 text-status-info border border-status-info/20"
                        : appt.status === "completed"
                        ? "bg-status-safe/10 text-status-safe border border-status-safe/25"
                  ...
                  <td className="text-right pr-4">
                    <div className="flex items-center justify-end gap-2">
                      ...
                      <button
                        onClick={() => handleStartScribe(appt.appointmentId, appt.patientId)}
                        className="bg-accent/10 border border-accent/30 text-accent text-[10px] font-bold px-2 py-0.5 rounded-sm hover:bg-accent/20 transition-colors"
                      >
                        Scribe
                      </button>
                    </div>
        ...
        <div className="bg-bg-surface rounded-sm border border-border-dim overflow-hidden">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left min-w-[640px]">
              <thead>
                <tr className="border-b border-border-dim text-[10px] font-sans font-bold text-text-tertiary uppercase tracking-wider h-10">
                  <th className="pl-4 whitespace-nowrap">ID</th>
                  <th className="whitespace-nowrap">NAME</th>
                  <th className="whitespace-nowrap hidden md:table-cell">GENDER/AGE</th>
                  <th className="whitespace-nowrap hidden sm:table-cell">BLOOD</th>
                  <th className="whitespace-nowrap">ALLERGIES</th>
                  ...
                  <tr key={p.id} className="hover:bg-bg-subtle/20 h-12 transition-colors">
                    ...
                    <td>
                      <button
                        ...
                        className="text-xs font-sans font-semibold text-text-primary hover:text-accent hover:underline text-left whitespace-nowrap"
                      >
                        {p.name}
                      </button>
                    </td>
                    <td className="text-xs font-sans text-text-secondary whitespace-nowrap hidden md:table-cell">
                      {p.gender === "F" ? "Female" : "Male"}, {p.age}y
                    </td>
                    <td className="text-xs font-mono text-text-secondary whitespace-nowrap hidden sm:table-cell">
                      {p.bloodGroup}
                    </td>
                    ...
                    <td className="hidden lg:table-cell">
                      <span className="inline-flex h-2 w-2 rounded-full bg-status-safe mr-1.5" />
                      <span className="text-xs font-sans text-text-secondary">Active</span>
                    </td>
                    <td className="text-right pr-4">
  ...
  function renderPatientDetailPage() {
    ...
    return (
      <div className="flex-1 overflow-y-auto bg-bg-base">
          ...
          <div className="flex flex-wrap gap-2 shrink-0">
            {profile.allergies.length > 0 && (
              <div className="flex gap-1.5 items-center">
                <span className="text-[10px] font-sans font-bold text-status-danger uppercase tracking-wider">
                  ALLERGIES:
                </span>
                {profile.allergies.map((all, i) => (
                  <span key={i} className="bg-status-danger/10 text-[10px] font-sans font-bold text-status-danger px-1.5 py-0.5 rounded-sm border border-status-danger/20 whitespace-nowrap">
                    {all}
                  </span>
                ))}
              </div>
            ...
            <button
              ...
              className="bg-accent text-bg-base text-xs font-bold px-3 py-1.5 rounded-sm hover:opacity-90 inline-flex items-center gap-1.5 shadow-sm whitespace-nowrap"
            >
        ...
        <div className="p-4 sm:p-5 md:p-8 grid grid-cols-1 lg:grid-cols-12 gap-4 sm:gap-5 lg:gap-8">
          
          {/* Left panel core analytics (60%) */}
          <div className="lg:col-span-7 space-y-5">
            
            {/* AI Summary Block */}
            <div className="bg-bg-surface p-4 md:p-6 rounded-sm border border-border-dim shadow-xs space-y-4">
              <div className="flex justify-between items-center pb-2 border-b border-border-dim">
                <div className="flex items-center gap-2">
            ...
            <div className="bg-bg-surface p-4 md:p-6 rounded-sm border border-border-dim shadow-xs space-y-5">
  ...
  function renderLiveScribeScreen() {
    ...
    return (
      <div className="flex-1 flex flex-col overflow-y-auto bg-bg-base select-text">
        ...
        <div className="bg-bg-surface border-b border-border-dim px-4 py-3 flex flex-col gap-3 shrink-0 sm:flex-row sm:items-center sm:justify-between sticky top-0 z-10">
          {/* Left: back + title */}
          <div className="flex items-start gap-3">
            <button 
              onClick={() => navigateTo("home")}
              className="p-1.5 rounded-sm border border-border-dim text-text-tertiary hover:text-text-primary bg-bg-base shrink-0 mt-0.5"
            >
              <ChevronLeft size={14} />
            </button>
            <div>
              <div className="flex items-center gap-2 flex-wrap">
                <h2 className="text-sm font-display font-semibold text-text-primary leading-tight">
                  AI Consultation Scribe
                </h2>
                <span className="text-[10px] font-bold text-status-safe bg-status-safe/10 px-2 py-0.5 rounded-sm border border-status-safe/25 uppercase shrink-0">
                  Connected
                </span>
              </div>
              <p className="text-[11px] text-text-secondary font-sans mt-0.5 leading-tight">
                Patient: <span className="font-semibold text-text-primary">{activePatient?.name || "Priya Mehta"}</span>
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2 flex-wrap">
            <div className="flex items-center gap-2 bg-text-primary/5 border border-border-dim/80 px-2.5 py-1.5 rounded-sm shrink-0">
              <span className={`h-2 w-2 rounded-full shrink-0 ${isRecording ? "bg-status-danger animate-pulse" : "bg-text-tertiary"}`} />
              <span className="font-mono text-[11px] font-bold text-text-primary whitespace-nowrap">
                {isRecording ? "LIVE" : "IDLE"} · {formatAudioTime(recordingSeconds)}
              </span>
            </div>
            ...
            {isRecording ? (
              <button 
                ...
                className="bg-status-warning text-bg-base text-xs font-bold px-3 py-1.5 rounded-sm hover:-translate-y-px transition-transform whitespace-nowrap"
              >
                Pause
              </button>
            ) : (
              <button 
                ...
                className="bg-accent text-bg-base text-xs font-bold px-3 py-1.5 rounded-sm hover:-translate-y-px transition-transform flex items-center gap-1.5 shadow-sm whitespace-nowrap"
              >
            ...
            <button
              ...
              className="hidden sm:inline-flex bg-text-primary text-bg-base text-xs font-bold px-3 py-1.5 rounded-sm hover:opacity-90 items-center gap-1 shadow-xs whitespace-nowrap"
            >
              Rx Draft <ChevronRight size={13} />
            </button>
          </div>
        </div>
        ...
        <div className="flex flex-col lg:flex-row lg:flex-1 lg:overflow-hidden lg:min-h-0">
          ...
          <div className="w-full lg:w-1/2 border-b lg:border-b-0 lg:border-r border-border-dim flex flex-col bg-bg-surface min-h-[280px] lg:overflow-hidden">
            <div className="p-3 border-b border-border-dim bg-bg-base/30 flex justify-between items-center shrink-0">
            ...
            <div className="p-4 lg:flex-1 lg:overflow-y-auto space-y-5 font-mono text-sm leading-relaxed select-text">
          ...
          <div className="w-full lg:w-1/2 flex flex-col bg-bg-base lg:overflow-hidden">
            <div className="p-3 border-b border-border-dim bg-bg-surface flex justify-between items-center shrink-0">
            ...
            {/* SOAP inputs layout */}
            <div className="p-4 lg:flex-1 lg:overflow-y-auto space-y-4">
  ...
  function renderPrescriptionWriterScreen() {
    ...
    return (
      <div className="flex-1 overflow-y-auto flex flex-col bg-bg-base select-text">
        ...
        <div className="bg-bg-surface border-b border-border-dim p-3 md:p-4 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 shrink-0 shadow-xs sticky top-0 z-10">
          <div className="flex items-center gap-3">
            <button 
            ...
            <div>
              ...
              <p className="text-[11px] text-text-secondary font-sans mt-0.5">
                For <span className="font-bold text-text-primary">{activePatient?.name}</span> · Age {activePatient?.age} ({activePatient?.gender})
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="bg-accent text-bg-base text-xs font-sans font-bold px-3 py-1.5 rounded-sm hover:opacity-90 inline-flex items-center gap-1 shadow-sm"
            >
        ...
        {/* Prescription layout — stacked on mobile, side-by-side on lg+ */}
        <div className="flex flex-col lg:flex-row">
          
          {/* LEFT PANEL: Editing Controls */}
          <div className="w-full lg:w-[55%] border-b lg:border-b-0 lg:border-r border-border-dim bg-bg-surface p-4 md:p-5 space-y-5">
          ...
          <div className="w-full lg:w-[45%] bg-bg-base p-4 md:p-6">
            ...
            {/* Mock physical sheet — natural height, no aspect ratio clipping */}
            <div className="bg-white text-gray-900 font-sans p-5 md:p-6 rounded-sm shadow-md border border-gray-200 w-full max-w-lg mx-auto space-y-4 select-text text-left">
              ...
              <div className="flex justify-between items-start border-b border-gray-300 pb-3">
                <div>
                  ...
                  <p className="text-[9px] text-gray-500 font-medium mt-0.5">
                    Sector 42, DLF Phase 5, Gurgaon DL
                  </p>
                  <p className="text-[9px] text-gray-500 font-mono">
                    REG: MCI/DL/2018/48291
                  </p>
                </div>
                <div className="text-right text-[9px] text-gray-600 font-mono space-y-0.5">
              ...
              <div className="grid grid-cols-2 gap-2 text-xs border-b border-gray-200 pb-3 bg-gray-50/50 p-2 rounded-xs">
                <div>
                  <span className="text-[8px] text-gray-400 font-bold block uppercase">PATIENT</span>
                  <div className="font-bold text-gray-800 text-[11px]">{activePatient?.name}</div>
                  <div className="text-[10px] text-gray-500">{activePatient?.gender} / {activePatient?.age} yrs &middot; {activePatient?.bloodGroup}</div>
                </div>

                <div>
                  <span className="text-[8px] text-gray-400 font-bold block uppercase">DOCTOR</span>
                  <div className="font-bold text-gray-800 text-[11px]">{currentDoctor.name}</div>
              ...
              <div className="space-y-3">
                <div>
                  <span className="text-[8px] text-gray-400 font-bold block uppercase">DIAGNOSES</span>
                  <div className="flex flex-wrap gap-1 mt-1">
                    {diagnosisTags.map((tag, i) => (
                      <span key={i} className="text-[9px] font-semibold bg-gray-100 px-1.5 py-0.5 rounded-sm text-gray-800 border border-gray-200">
                        {tag}
                      </span>
                    ))}
                  </div>
                ...
                <div className="text-base font-bold font-serif text-gray-800 leading-none">Rx</div>

                {/* Medicines layout */}
                <div className="overflow-x-auto">
                  <table className="w-full text-left text-xs">
                    <thead>
                      <tr className="border-b border-gray-300 pb-1 text-[8px] text-gray-400 font-bold uppercase tracking-wider">
                        <th className="pb-1">MEDICINE / DOSING</th>
                        <th className="pb-1">TIMING</th>
                      ...
                      {prescribedMeds.map((med, idx) => (
                        <tr key={idx} className="h-9">
                          <td className="pt-1 pb-1">
                            <span className="font-bold text-gray-800 text-[11px] block">{med.name} {med.dosage}</span>
                            <span className="text-[9px] text-gray-500 block italic">"{med.instructions}"</span>
                          </td>
                          <td className="font-mono text-[10px] text-gray-600 font-bold">{med.timing}</td>
                          <td className="text-right font-mono font-bold text-gray-800 text-[10px]">{med.durationDays}d</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
              ...
              {testChips.length > 0 && (
                <div className="border-t border-gray-200 pt-3">
                  <span className="text-[8px] text-gray-400 font-bold block uppercase mb-1">LAB TESTS ORDERED</span>
                  <div className="flex flex-wrap gap-1">
                    {testChips.map((test, index) => (
                      <span key={index} className="text-[9px] font-mono font-bold bg-gray-50 border border-gray-200 rounded-sm px-1.5 py-0.5 text-gray-700">
                        • {test}
                      </span>
                    ))}
                  </div>
              ...
              <div className="border-t border-gray-200 pt-3">
                <span className="text-[8px] text-gray-400 font-bold block uppercase">DIET RECOMMENDATIONS</span>
                <p className="text-[10px] text-gray-600 leading-normal mt-0.5 italic">
                  "{dietInstructions}"
                </p>
              </div>
              ...
              <div className="border-t border-gray-200 pt-3 flex justify-between items-end text-xs text-gray-500">
                <div>
                  <span className="text-[8px] text-gray-400 font-bold block uppercase">FOLLOW-UP</span>
                  <span className="font-bold text-gray-800 text-[11px]">Return in {followUpDays} days</span>
                </div>
                
                {/* Simulated Signature */}
                <div className="text-right font-serif leading-none pr-1">
                  <div className="italic text-gray-400 text-[8px] mb-1">Authenticated electronically</div>
                  <div className="text-[12px] font-bold text-gray-800">Dr. Rajesh Sharma</div>
                  <div className="text-[8px] text-gray-500 mt-0.5 font-sans">MD Specialty Cardiologist</div>
                </div>
              </div>
  ...
  function renderClinicalNotesArchive() {
    ...
    return (
      <div className="flex-1 flex flex-col md:flex-row overflow-hidden bg-bg-base">
        {/* Left Side: Archive index — full width on mobile, fixed sidebar on md+ */}
        <div className="w-full md:w-[320px] lg:w-[360px] border-b md:border-b-0 md:border-r border-border-dim flex flex-col bg-bg-surface overflow-hidden shrink-0 md:max-h-full max-h-[220px]">
  ...
  function renderSlotSaverDashboard() {
    return (
      <div className="flex-1 p-4 sm:p-5 md:p-8 space-y-4 md:space-y-6 overflow-y-auto bg-bg-base">
        <div>
          <h1 className="text-xl md:text-2xl font-display font-medium text-text-primary">
            SlotSaver Revenue Protection
          </h1>
          <p className="text-xs font-sans text-text-secondary mt-1">
            Re-allocating unfulfilled clinic slots using waitlists and auto-outreach.
          </p>
        </div>

        {/* KPI strip — 2-col mobile, 4-col desktop */}
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4 bg-bg-surface p-4 md:p-6 rounded-sm border border-border-dim shadow-xs">
          <div>
            <span className="text-[9px] font-sans font-bold tracking-widest text-text-tertiary uppercase block leading-tight">
              Revenue Recovered
            </span>
            ...
            <span className="text-[10px] font-sans text-status-safe block mt-1">
          ...
          <div>
            ...
            <span className="text-[10px] font-sans text-text-tertiary block mt-1">
              Avg benchmark: 60%
            </span>
          </div>
          ...
          <div>
            ...
            <span className="text-2xl md:text-3xl font-mono font-bold text-status-warning block mt-1.5">
              1 Live
            </span>
            <span className="text-[10px] font-sans text-text-secondary block mt-1">
              10:30 AM slot
            </span>
          </div>
        </div>
          ...
          <div className="bg-bg-surface p-4 md:p-6 rounded-sm border border-border-dim shadow-xs space-y-4">
            ...
            <div className="p-4 bg-bg-base border border-border-dim rounded-sm">
              <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-2 text-xs">
                <div>
                  <span className="font-bold text-text-primary">Slot: 10:30 AM</span>
                  <span className="text-[10px] text-text-tertiary block mt-0.5">Cardiology (Cancelled by Dev Shah)</span>
                </div>
                <span className="text-[10px] font-bold text-status-warning bg-status-warning/10 px-2 py-0.5 rounded-sm uppercase tracking-wider self-start sm:self-auto">
                  Outreach sent
                </span>
              </div>

              <div className="mt-4 pt-4 border-t border-border-dim/40 space-y-2 text-xs font-sans text-text-secondary leading-normal">
                <p>• Waitlisted: <span className="font-bold text-text-primary">Priya Mehta</span> (high suitability match)</p>
                <p>• WhatsApp dispatched: <span className="font-mono">8:42 AM</span></p>
                <p>• Timer: <span className="font-mono font-bold text-text-primary">02m 44s</span> (awaiting confirm)</p>
              </div>
            </div>
          </div>

          {/* Tomorrow's risk — card list on mobile, table on md+ */}
          <div className="bg-bg-surface p-4 md:p-6 rounded-sm border border-border-dim shadow-xs space-y-4">
            <span className="text-[10px] font-sans font-bold tracking-widest text-text-tertiary uppercase block">
              TOMORROW NO-SHOW RISKS
            </span>

            {/* Mobile cards */}
            <div className="space-y-3 md:hidden">
              {[
                { name: "Rohit Sharma", time: "10:00 AM", risk: "92%", riskClass: "text-status-danger", status: "✓ Confirmed", statusClass: "text-status-safe", action: null },
                { name: "Neha Gupta", time: "12:30 PM", risk: "74%", riskClass: "text-status-warning", status: null, action: "Queue Outreach" }
              ].map((row, i) => (
                <div key={i} className="p-3 bg-bg-base rounded-sm border border-border-dim text-xs">
                  <div className="flex justify-between items-start">
                    <span className="font-bold text-text-primary">{row.name}</span>
                    <span className="font-mono text-text-secondary">{row.time}</span>
                  </div>
                  <div className="flex justify-between items-center mt-2">
                    <span className={`font-bold ${row.riskClass}`}>{row.risk} risk</span>
                    {row.action ? (
                      <button onClick={() => showToast("Dispatched priority SMS.")}
                        className="bg-accent/10 text-accent font-bold text-[10px] px-2 py-0.5 border border-accent/25 rounded-sm"
                      >{row.action}</button>
                    ) : (
                      <span className={`font-semibold ${row.statusClass}`}>{row.status}</span>
                    )}
                  </div>
                </div>
              ))}
              ...
              <table className="w-full text-left">
                <thead>
                  <tr className="border-b border-border-dim pb-1.5 text-[9px] font-sans font-bold text-text-tertiary uppercase tracking-wider">
                    ...
                    <th className="pb-2 text-right">INTERVENTION</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-border-dim">
                  <tr className="h-10 text-xs font-sans">
                    ...
                    <td className="text-status-danger font-bold">92%</td>
                    <td className="text-right font-medium text-status-safe">✓ Pre-called</td>
                  </tr>
                  <tr className="h-10 text-xs font-sans">
                    <td className="font-bold text-text-primary">Neha Gupta</td>
                    <td className="font-mono text-text-secondary">12:30 PM</td>
                    <td className="text-status-warning font-semibold">74%</td>
                    <td className="text-right">
                      <button onClick={() => showToast("Dispatched priority SMS confirm ticket.")}
  ...
  function renderSettingsPage() {
    return (
      <div className="flex-1 p-4 sm:p-6 md:p-8 space-y-4 md:space-y-6 overflow-y-auto bg-bg-base select-none">
        ...
        <div className="max-w-2xl bg-bg-surface p-4 md:p-6 rounded-sm border border-border-dim shadow-xs space-y-6">
