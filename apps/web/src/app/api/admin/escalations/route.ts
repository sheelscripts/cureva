  ...
  try {
    ...
    try {
      const { data: sessions } = await supabaseAdmin
        ...
        .select('id, slot_id, filled_by_patient')
        .in('id', sessionIds)
        .limit(100);
      ...
      // recovery_sessions / slots may not exist — use defaults
    }

    const result = data.map(e => {
      const session = (data as Array<{session_id?: string}>).find(d => d.session_id === e.session_id);
      const slotInfo = session ? slotMap[session.session_id || ''] : null;
      return {
        id: e.id,
