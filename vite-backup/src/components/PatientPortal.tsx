...
export default function PatientPortal({ currentSubView, onNavigateToView, resetTrigger }: PatientPortalProps) {
  ...
  return (
    <div className="flex-1 flex flex-col h-screen overflow-hidden bg-bg-base text-text-primary select-none">
      
      {/* GLOBAL NOTIFICATION BANNER / TOP BAR */}
      <div className="px-6 py-2 bg-text-primary text-bg-base flex justify-between items-center text-[11px] font-mono tracking-wide">
        <div className="flex items-center gap-1.5 font-medium">
          <span className="h-1.5 w-1.5 bg-status-safe rounded-full animate-ping" />
          ACTIVE TRIAGE CONTEXT — PRIYA MEHTA (34F) · SECTOR 12
        </div>
        <div className="flex items-center gap-4">
          <span className="opacity-75">BLOOD TYPE: <strong className="text-status-warning">{patient.bloodGroup}</strong></span>
          <span className="opacity-75">ALLERGIES: <strong className="text-status-danger">{patient.allergies.join(", ")}</strong></span>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        {currentSubView === "triage" && (
          <div className="p-4 sm:p-6 md:p-8 max-w-7xl mx-auto space-y-6 sm:space-y-8 animate-fade-in">
            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
              <div>
                <h1 className="text-3xl font-display font-medium tracking-tight text-text-primary">
                  Good morning, <span className="font-serif italic text-text-secondary">{patient.name}</span>
                </h1>
                <p className="text-xs text-text-secondary font-mono tracking-wide mt-1.5">
                  Monday, January 15 · Air quality: Moderate · City Clinic Area (2.1 km)
                </p>
              </div>
              <div className="flex gap-2">
                <button 
                  onClick={() => onNavigateToView("book")}
                  className="px-4 py-2 border border-border-base hover:border-text-primary hover:bg-bg-subtle text-xs font-semibold rounded-sm transition-all flex items-center gap-1"
                >
                  Book Instant Visit <ArrowRight size={13} />
                </button>
              </div>
            </div>
        ...
        {currentSubView === "book" && (
          <div className="p-4 sm:p-6 md:p-8 max-w-4xl mx-auto space-y-6 sm:space-y-8 animate-fade-in">
            <div>
              <h1 className="text-2xl font-display font-medium tracking-tight text-text-primary">
                Book structured consultation
              </h1>
              <p className="text-xs text-text-secondary mt-1">
                Lock clinical slots instantly with certified physicians across City Network Clinics
              </p>
            </div>

            {/* Step Wizard Progress Header */}
            <div className="bg-bg-surface border border-border-dim p-4 rounded-sm flex justify-center">
              <div className="flex items-center gap-5">
                {[
                  { step: 1, label: "Select Specialty" },
                  { step: 2, label: "Identify Doctor" },
                  { step: 3, label: "Verify & Confirm" }
                ].map((s) => (
                  <div key={s.step} className="flex items-center gap-2">
                    ...
                    {s.step < 3 && <div className="h-0.5 w-12 bg-border-base" />}
                  </div>
                ))}
              </div>
            </div>
        ...
        {currentSubView === "dashboard" && (
          <div className="p-4 sm:p-6 md:p-8 max-w-7xl mx-auto space-y-6 sm:space-y-8 animate-fade-in">
            ...
            <div className="bg-bg-surface border border-border-dim p-6 rounded-sm flex flex-col md:flex-row md:items-center justify-between gap-6">
              <div className="flex items-center gap-4">
                <div className="w-12 h-12 rounded-full bg-bg-subtle border border-border-base flex items-center justify-center text-text-primary font-serif italic text-lg font-medium">
                  {patient.name.charAt(0)}
                </div>
                <div>
                  <h2 className="text-lg font-bold text-text-primary">{patient.name}</h2>
                  <p className="text-xs text-text-secondary font-mono">Patient Account: #{patient.id} · Member Since Nov 2024</p>
                </div>
              </div>
              <div className="flex flex-wrap gap-2.5 sm:gap-4 text-xs">
              ...
              <div className="lg:col-span-8 space-y-8">
                ...
                <div className="space-y-3">
                  ...
                  <div className="bg-bg-surface border border-border-dim rounded-sm overflow-hidden text-xs">
                    <div className="grid grid-cols-12 p-3 bg-bg-subtle text-text-tertiary font-mono tracking-wider border-b border-border-dim text-[10px] uppercase">
                    ...
                    <div className="divide-y divide-border-dim">
                      {labsList.slice(0, 3).map((lab) => (
                        <div key={lab.id} className="grid grid-cols-12 p-4 items-center hover:bg-bg-subtle/50 transition-colors">
                          <div className="col-span-3 font-mono text-text-secondary">{lab.date}</div>
                          <div className="col-span-5 font-bold text-text-primary">{lab.name}</div>
                          ...
                          <div className="col-span-2 text-right">
                            <button
                              ...
                              className="text-text-primary hover:underline font-semibold font-mono"
                            >
                              Analyze
                            </button>
                          </div>
        ...
        {currentSubView === "prescriptions" && (
          <div className="p-4 sm:p-6 md:p-8 max-w-5xl mx-auto space-y-6 sm:space-y-8 animate-fade-in animate-slide-up">
            <div>
              <h1 className="text-2xl font-display font-medium tracking-tight text-text-primary">
                Verified Prescriptions
              </h1>
              <p className="text-xs text-text-secondary mt-1">
                Medical Council (MCI) certified documents registered under your unique healthcare dossier
              </p>
            </div>
          ...
          <div className="p-4 sm:p-6 md:p-8 max-w-6xl mx-auto space-y-6 sm:space-y-8 animate-fade-in">
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
              <div>
                ...
                <p className="text-xs text-text-secondary mt-1">
                  Lab verified biochemical measurements with grounded diagnostic range comparisons
                </p>
              </div>

              {/* Filtering pill bar */}
              <div className="flex gap-1.5 bg-bg-surface p-1 rounded-sm border border-border-dim">
          ...
          <div className="p-4 sm:p-6 md:p-8 max-w-5xl mx-auto space-y-6 sm:space-y-8 animate-fade-in">
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
              <div>
                ...
                <p className="text-xs text-text-secondary mt-1">
                  A structured chronological view mapping appointments, prescriptions, and clinical tests
                </p>
              </div>
            ...
            {/* Timelines content list view */}
            <div className="bg-bg-surface border border-border-dim p-6 rounded-sm">
        ...
        {currentSubView === "ask" && (
          <div className="p-4 sm:p-6 md:p-8 max-w-5xl mx-auto space-y-6 sm:space-y-8 animate-fade-in font-sans">
            <div>
              <h1 className="text-2xl font-display font-medium tracking-tight text-text-primary">
                AI Health Assistant
              </h1>
              <p className="text-xs text-text-secondary mt-1">
                Grounded consultations based on your actual medical history, tests ordered and Active prescriptions.
              </p>
            </div>

            {/* Context grounding banner */}
            <div className="p-4 bg-bg-subtle border border-border-base rounded-sm flex items-center justify-between text-xs text-text-secondary">
              <span className="flex items-center gap-1.5 font-semibold text-text-primary tracking-wider uppercase text-[10px]">
                <ShieldCheck size={14} className="text-status-safe animate-pulse-dot" />
                Active Dataset Grounded (Jan 15 Update)
              </span>
              <span>DPDP Compliance Certified · Clinic Node ID CITY-DL-01</span>
            </div>

            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
              
              {/* Main Dialogue Box Area */}
              <div className="lg:col-span-8 bg-bg-surface border border-border-dim rounded-sm h-[580px] flex flex-col overflow-hidden">
                
                {/* Chat header */}
                <div className="p-4 border-b border-border-dim bg-bg-subtle flex justify-between items-center text-xs text-text-tertiary">
                  <span>Chat initialized with Priya Mehta's file context</span>
                  <button 
                    ...
                    className="text-[10px] font-mono uppercase bg-bg-base border border-border-base px-2 py-0.5 rounded-sm hover:text-text-primary"
                  >
                    Clear chat
                  </button>
                </div>

                {/* Question bubble list */}
                <div className="flex-1 overflow-y-auto p-6 space-y-6">
                  {askMessages.map((msg, index) => (
                    ...
                    >
                      <div className={`flex gap-3 max-w-[85%] ${msg.role === "user" ? "flex-row-reverse" : "flex-row"}`}>
                        ...
                        {msg.role === "ai" && (
                          <div className="w-6 h-6 rounded-sm bg-text-primary text-bg-base flex items-center justify-center text-[10px] font-bold shrink-0 uppercase">
                            AI
                          </div>
                        )}
                        ...
                        <div className="space-y-1">
                          <div 
                            className={`p-4 rounded-sm text-xs leading-relaxed border ${
                              msg.role === "user" 
                                ? "bg-text-primary text-bg-base border-text-primary" 
                                : "bg-bg-subtle text-text-primary border-border-dim"
                            }`}
                          >
                            ...
                            {msg.role === "ai" && !msg.isStreaming && (
                              <div className="mt-3 text-[10px] font-mono text-text-tertiary flex items-center gap-1 bg-bg-base border border-border-base w-max px-2 py-0.5 rounded-sm">
                                <Info size={11} className="text-text-secondary" />
                                <span>Source: Grounded in prescription RX-441 & Lipid results (LAB-202)</span>
                              </div>
                            )}
                          </div>
                          
                          <span className="text-[9px] font-mono text-text-tertiary block text-right mt-1">{msg.timestamp}</span>
                        </div>
                  ...
                  {isAskTyping && (
                    <div className="flex justify-start">
                      <div className="flex gap-3 max-w-[85%]">
                        <div className="w-6 h-6 rounded-sm bg-text-primary text-bg-base flex items-center justify-center text-[10px] font-bold shrink-0 animate-pulse">
                          AI
                        </div>
                        <div className="p-3 bg-bg-subtle text-text-tertiary border border-border-dim rounded-sm text-xs">
                          <span className="shimmer h-[1bh] w-48 block">Interpreting clinical references...</span>
                        </div>
                      </div>
                    </div>
                  )}
                ...
                <form 
                  ...
                  className="p-3 border-t border-border-dim bg-bg-surface flex gap-2"
                >
                  <input 
                    type="text"
                    value={askInput}
                    onChange={(e) => setAskInput(e.target.value)}
                    placeholder="Ask standard questions: 'Is my blood pressure under control?' or 'When should I take Atorvastatin?'"
                    className="flex-1 bg-bg-base text-text-primary border border-border-base px-3.5 py-2 text-xs rounded-sm focus:outline-none focus:border-text-primary font-sans placeholder-text-tertiary"
                  />
                  <button 
                    type="submit"
              ...
              {/* RIGHT QUICK ASSISTANT SIDE COLUMN */}
              <div className="lg:col-span-4 space-y-4">
                ...
                <div className="space-y-2.5">
                  {[
                    "What does my Lipid Panel result mean?",
                    "When should I take Atorvastatin 10mg?",
                    "Is my stage 1 hypertension stable?",
                    "What lifestyle changes did Dr. Sharma instruct?"
                  ].map((qst, idx) => (
                    <button
                      key={idx}
                      onClick={() => handleSendAsk(qst)}
                      className="w-full text-left p-3 bg-bg-surface border border-border-dim hover:border-text-primary rounded-sm text-xs text-text-secondary hover:text-text-primary transition-all duration-150 cursor-pointer"
                    >
                      <span>{qst}</span>
                    </button>
                  ))}
                ...
                <div className="p-4 bg-bg-subtle border border-border-base rounded-sm text-xs text-text-tertiary space-y-2 leading-relaxed">
                  <span className="text-[10px] font-mono uppercase font-bold text-text-primary block">Clinical Safety warning:</span>
                  <p className="text-[11px]">
                    The clinical AI assistant is a diagnostic routing and document translation mechanism. It operates under standard medical protocols but does NOT substitute actual hospital emergency procedures. For high chest pains, visit City Hospital immediately.
                  </p>
                </div>
              </div>
